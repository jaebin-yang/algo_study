...
public class SW_PokerGame_9760 {
	...
	public static void main(String[] args) throws Exception{
		...
		for (int t = 1; t <= T; t++) {
			suitCnt = new int[4];
			rankCnt = new int[14]; // 0 dummy rankCnt[3] => 2
			...
			for (int i = 1; i <= 9; i++) {
				// rankCnt[3]... 4 5 6 7
				if( rankCnt[i]*rankCnt[i+1]*rankCnt[i+2]*rankCnt[i+3]*rankCnt[i+4] == 1 ) Straight = true;
			}
			
			for (int i = 1; i <= 9; i++) {
                // 랭크의 범위 내에서 5개가 연속일 경우 (모두 1인 경우)
                if (rankCnt[i]*rankCnt[i + 1]*rankCnt[i + 2]*rankCnt[i + 3]*rankCnt[i + 4]  == 1) Straight = true;
            }
